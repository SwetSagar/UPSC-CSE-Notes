		...
		try {
			...
			if (e.status === 429) {
				new import_obsidian2.Notice(
					"GPT-3 Rate limit error: please try again soon."
				);
...
var PluginModal = class extends import_obsidian3.Modal {
	...
	onOpen() {
		...
		const history_toggle = new import_obsidian3.ToggleComponent(
			history_toggle_container
		);
		const history_dropdown = new import_obsidian3.DropdownComponent(
			container
		);
		...
		const tempSetting = new import_obsidian3.Setting(container)
			.setName("Temperature")
		...
		const tokenSetting = new import_obsidian3.Setting(container)
			.setName("Tokens")
			...
			.addDropdown((dropdown) => {
				...
				dropdown.onChange((change) => {
					...
					tokenSetting.settingEl.style.display =
						models[this.plugin.settings.model] === "chat"
							? "none"
							: "";
				});
				dropdown.setValue(this.plugin.settings.model);
		...
		const cancelButton = new import_obsidian3.ButtonComponent(
			buttonContainer
		);
		...
		this.generateButton = new import_obsidian3.ButtonComponent(
			buttonContainer
		);
	...
	async handleGenerateClick() {
		...
		if (!token) {
			new import_obsidian3.Notice(
				"Please enter your OpenAI token in the plugin settings"
			);
			this.generateButton.setDisabled(false);
			this.generateButton.setButtonText("Generate Notes");
			return;
...
var PreviewModal = class extends import_obsidian4.Modal {
	...
	onOpen() {
		...
		this.previewText = "";
		this.previewTextArea = new import_obsidian4.TextAreaComponent(
			container
		);
		...
		const cancelButton = new import_obsidian4.ButtonComponent(
			buttonContainer
		);
		cancelButton.buttonEl.style.backgroundColor = "#b33939";
		...
		this.regenerateButton = new import_obsidian4.ButtonComponent(
			buttonContainer
		);
		...
		const addToDocumentButton = new import_obsidian4.ButtonComponent(
			buttonContainer
		);
...
var prompts_default = {
	...
	postfix: ["with sources", "with footnotes", "with comments", "with quotes"],
};
