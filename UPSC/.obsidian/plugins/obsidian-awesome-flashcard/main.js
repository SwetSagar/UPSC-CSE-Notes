...
async function mdToHtml(plugin, content) {
  ...
  return `<style>@import url("_obsidian_card.css");</style><div style="text-align: left;">${html}</div>`;
}
...
var obsidian_card_default = `body {
  /*
...
.canvas-controls button:disabled svg {
  color: var(--text-faint);
}
...
.canvas-controls-group button.is-active {
  color: var(--color-accent);
}
...
.canvas-node:not(.is-editing) .canvas-node-container:active {
  cursor: grabbing;
}
.canvas-node-label {
  position: absolute;
...
.canvas-node-resizer {
  ...
  width: calc(var(--resizer-size) * var(--zoom-multiplier) * 0.5);
}
...
.is-mobile .canvas-node.is-focused .canvas-node-resizer[data-resize='topright'],
.is-mobile .canvas-node.is-focused .canvas-node-resizer[data-resize='bottomright'],
.is-mobile .canvas-node.is-focused .canvas-node-resizer[data-resize='topleft'],
.is-mobile .canvas-node.is-focused .canvas-node-resizer[data-resize='bottomleft'] {
  pointer-events: all;
  width: 20px;
  height: 20px;
  display: block;
...
.is-mobile .canvas-node.is-focused .canvas-node-connection-point::after,
.canvas.is-connecting .canvas-node:hover .canvas-node-connection-point::after,
...
.canvas-edges path.canvas-display-path {
  pointer-events: none;
  stroke-width: calc(3 * var(--zoom-multiplier));
...
.canvas-edges path.canvas-interaction-path {
  ...
  stroke-width: calc(24 * var(--zoom-multiplier));
...
.canvas-edges path.canvas-interaction-path:active {
  cursor: grabbing;
}
.canvas-edges polyline.canvas-path-end {
  pointer-events: none;
  stroke: rgb(var(--canvas-color));
...
.canvas:not(.is-connecting) .canvas-edges g:hover path.canvas-display-path {
  stroke-width: calc(5.5 * var(--zoom-multiplier));
}
...
.canvas-submenu.mod-vertical {
  flex-direction: column;
}
.canvas-submenu.mod-vertical .canvas-menu-group:not(:last-child) {
  margin-bottom: var(--size-2-2);
}
.canvas-submenu.mod-right {
  transform: none;
  left: calc(100% + 5px);
  top: -1px;
}
.canvas-menu-group-label {
  padding: var(--size-2-3) var(--size-2-3) var(--size-2-2);
  font-size: var(--font-ui-smaller);
  color: var(--text-muted);
  font-weight: var(--font-semibold);
}
